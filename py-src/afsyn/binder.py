...
from afsyn.unit import UnitMgr
from afsyn.bipartite import bipartite_matching

debug = False


def reg_bind1(dfg, unit_mgr, var_list) :
    # var_list に含まれる変数にレジスタ割り当てを行う．
...
def bind_register(dfg, unit_mgr) :
    ...
    for var in dfg.var_list :
        inode = var.src
        if inode.is_memsrc and var.start == var.end :
            var.bind(inode.unit)
        else :
            var_list.append(var)
    var_list.sort()
...
def alloc_op1_selecter(dfg, unit_mgr) :
    ...
    for node in dfg.op1node_list :
        ...
        for i, inode in enumerate(node.fanin_list) :
            if inode.next_step == node.cstep and inode.is_memsrc :
                unit = inode.unit
            else :
                var = inode.var
                unit = var.unit
