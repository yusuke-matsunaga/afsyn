    ...
    def eval_on(self, step) :
        if step in self.__block.bank_dict :
            ...
            if debug_out :
                ...
                debug_out.write('  oreg_value = {}\n'.format(self.__oreg_value))

    ### @brief シミュレーション時刻を進める．
    def sync(self) :
        if self.__next_value is not None or self.__oreg_value is not None :
            self.__value = self.__oreg_value
            self.__oreg_value = self.__next_value
            self.__next_value = None
            if debug_out :
...
class UnitMgr :
    ...
    def simulate(self, ivals, oaddr_list, debug) :
        ...
        for step in range(self.total_step + 1) :
            if debug_out :
